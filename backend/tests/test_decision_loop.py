...
def test_replay_recommendation_and_explicit_decision() -> None:
    ...
    replay = client.post("/api/v1/decision-loop/demo/replay", headers=headers)
    assert (
        replay.status_code == 200
        and replay.json()["scenario_id"] == "scenario_fixed_north_sector_v1"
    )
    recommendation = client.post("/api/v1/decision-loop/recommendations", headers=headers)
    ...
    assert (
        client.post("/api/v1/decision-loop/demo/replay", headers=headers).json()["scenario_id"]
