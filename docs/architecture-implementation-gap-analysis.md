...
| Two queues | Scoped response and verification queue APIs exist. | Partial | Queue ownership, deadlines, evidence/incident-origin metadata, and verification completion lifecycle are absent. |
...
| Commander-to-task authority | Approval/reject is durable; approval creates a linked queue item; a separate explicit queue approval creates the task. | Partial | No modify/override path or field outcome linked back to recommendation. |
...
| 3 — Operations | Resources with capability/readiness expiry, response + verification queues, routes, no-double-booking, task lifecycle, local durable test. | Capacity, queue lifecycle metadata, multi-client race drill, field outcome. |
